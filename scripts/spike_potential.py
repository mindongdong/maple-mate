...
def _inspect(label: str, records: list[dict]) -> None:
    ...
    if not records:
        print(
            "  (기록 0건 — 재설정/큐브 1회 후 재시도하면 등업 의미를 확정할 수 있어요)"
        )
        return
    ...
    # 결과 문자열 분포
    print(
        f"  결과 분포: {dict(Counter(r.get('item_upgrade_result') for r in records))}"
    )
    ...
    for r in success:
        ...
        rose = (
            bt is not None and at is not None and _ORDER.get(at, 0) > _ORDER.get(bt, 0)
        )
        ...
        print(
            f"        ④ before[0].grade={first}  (from-등급이 {bt} 와 일치하는지 확인)"
        )

    guaranteed = [r for r in records if r.get("upgrade_guarantee")]
    if guaranteed:
        print(
            f"  ③ 천장(upgrade_guarantee=true) 레코드의 결과 문자열: "
...
async def main() -> None:
    ...
    date_iso = (
        sys.argv[1] if len(sys.argv) > 1 else datetime.now(KST).date().isoformat()
    )
