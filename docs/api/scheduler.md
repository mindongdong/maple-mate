...
> ### ✅ 필드 의미 실측 (2026-06-27, 9캐릭 전수 — 봇 카테고리 파생 근거, [ADR-0013](../adr/0013-scheduler-field-derived-categories.md))
> 봇이 콘텐츠를 **이름 하드코딩 없이** 분류하려고 실 응답 필드를 전수 관측했다. 확정 사실:
> - **`type`** = `"contents"` 또는 `"quest"`. **퀘스트(`quest`)는 `now/max`가 무의미**(일일 퀘스트가 다수 `0/100`·`quest_state=1`) — 완료는 `quest_state`로만 본다(`"2"`완료/`"1"`미완료/`"0"`기타). "0/100 텍스트 벽"의 정체가 이것.
> - **`max_count` 의 의미가 콘텐츠 종류를 가른다**(type=contents 한정): **`0`=점수형(상한 없음)** — 길드 지하수로·플래그 레이스·주간 미션 포인트·에픽 던전·**무릉도장**(⚠️ `1/1` 아님, 실측 `max=0`); **`1`=완료/미완료**(에르다 스펙트럼·무토·체이서 등 주간 보스성); **`>1`=회수형**(몬스터파크 `2/14`).
> - **`quest_state="0"`(기타)** 일일퀘 관측(카르시온·탈라하트·기어드락, `0/0`) = 미해금 엔드게임 지역 추정 → 봇은 표시·집계 제외.
> - **보스 `cycle`** = `bossDaily`/`bossWeekly`/`bossMonthly` **3종뿐**. ⚠️ "시즌 보스"(메이린)도 `cycle=bossWeekly`로 와서 **별도 "시즌" 분류 불가**.
> - **보스 `difficulty`는 영문 enum**: `easy/normal/hard/chaos`(+`extreme`) — `노말/하드` 같은 한글 아님. 표시 시 매핑 필요.
> - **`weekly_boss_clear_limit_count` = `12`**(관측 `12` 또는 `0`). `(0,0)` 캐릭 존재 → 봇은 `0`/이상값이면 **12 폴백**.
