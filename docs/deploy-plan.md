...
> **✅ 그릴링 확정 (2026-06-10)** — 100서버 × 5명 × 10명령/일 시나리오 기준으로 단계 2 전체를
> 선행 결정(D1~D8). 실행 SSOT 는 [scale-tuning-work-order.md](scale-tuning-work-order.md).
> 결정 과정에서 원안 2건 정정: **2-2 는 단순 상향이 아니라 키별 버킷**(개인키 5/s 한도 충돌 —
> [ADR-0004](adr/0004-per-key-throttle-buckets.md)), **2-5 는 fetched_at 90일이 아니라 date 기준 400일**
> (불변 과거 캐시 삭제 시 개인키 재조회 폭증). 단계 3 선행은 기각.
>
> **✅ 코드 항목 구현 완료 (2026-06-10, [PR #14](https://github.com/mindongdong/maple-mate/pull/14))** —
> 2-2~2-6 + 부하 시뮬 검증([test_load_simulation.py](../tests/test_load_simulation.py)).
> 남은 것은 **운영자 작업 2-1·2-7**(+ 서비스단계 승인 후 `NEXON_THROTTLE=0.02` 설정).
> 아래 병목 분석·코드 링크는 작성 시점(구현 전) 기준 — as-built 는 작업지시서 §7.
...
| # | 작업 | 위치 | 효과 | 상태 |
|---|---|---|---|---|
| 2-1 | **넥슨 서비스단계 등록** | 넥슨 콘솔 | 한도 5→500/초 | ⏳ 운영자(공개 홍보 전) |
| 2-2 | ~~전역 스로틀 상향~~ → **키별 스로틀 버킷** (앱키=`NEXON_THROTTLE` env, 개인키=0.2s 고정 — [ADR-0004](adr/0004-per-key-throttle-buckets.md)) | [client.py](../maple_mate/nexon/client.py) | 처리량 12배+ · 이력류 cold 의 전역 독점 소멸 | ✅ PR #14 |
| 2-3 | **CPU 작업 `to_thread` 오프로딩** (Pillow·마르코프) | 명령 계층 호출부 6지점 | 동시성 | ✅ PR #14 |
| 2-4 | **사용자별 쿨다운** (명령군별 차등: 이력 30s·스펙 10s·설정 5s) | [bot/cooldowns.py](../maple_mate/bot/cooldowns.py) | 공개 필수 | ✅ PR #14 |
| 2-5 | **`history_cache` prune** (date 기준 400일) | [history/service.py](../maple_mate/history/service.py) → 운영요약 잡 편승 | DB 보호 | ✅ PR #14 |
| 2-6 | **최신 스펙 단기 캐시** (TTL 30분, 클라이언트 레벨) | [client.py `_spec`](../maple_mate/nexon/client.py) | 반복조회 | ✅ PR #14 |
| 2-7 | **인스턴스 등급 상향** (0.5→2 vCPU) | Render | CPU 여유 | ⏳ 운영자(공개 홍보 전) |

*→ verify: 동시 명령 20건 부하 시 큐 누적 없이 응답 / 어뷰즈 스팸이 전체를 막지 않음 / `history_cache` 행수 상한 유지.*
*✅ 코드화 완료 — [test_load_simulation.py](../tests/test_load_simulation.py)(동시 20명령 비차단·개인키 5/s 준수·무유실) + 항목별 단위테스트가 CI 에서 상시 검증.*
...
- ~~**`history_cache` 무한 증가**~~ — 단계 2-5(date−400일 prune, PR #14)로 해소.
