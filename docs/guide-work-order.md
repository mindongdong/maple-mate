...
> **상태: 구현 완료 (2026-06-16) · [PR #24](https://github.com/mindongdong/maple-mate/pull/24) OPEN.** `/grill-me` 12문으로 아래를 확정한 뒤 그대로 구현했다. 실제 반영 내역·계획 대비 차이는 §구현 결과(as-built) 참조.
...
> 문구 2차 수정(구현 후): 제목/온보딩의 `메이플메이트`→`메이트`, `길드원들`→`디스코드 채널 유저들`(`메이플스토리`는 유지). 위 §결정 #3·§"본문 문구는 구현 시 다듬을 수 있으나"와 일관.
...
**실제 반영 — [PR #24](https://github.com/mindongdong/maple-mate/pull/24), 2커밋** (`docs:` 작업지시서 + `feat:` 구현). 브랜치 `feat/guide-command` ← `main`.

| 파일 | 상태 | 내용 |
|---|---|---|
| `maple_mate/guide/__init__.py` | 신규 | 빈 패키지 |
| `maple_mate/guide/commands.py` | 신규 | `build_guide_embed()` 순수 함수(온보딩 + 6그룹) + 쿨다운 없는 `setup(bot)` |
| `maple_mate/bot/core.py` | 수정 | `핑` 정의 삭제 · 도메인 import에 `setup_guide` 추가 · `_register_commands()` 끝에 `setup_guide(self)` 배선 |
| `tests/test_cooldowns.py` | 수정 | `test_ping_has_no_cooldown` → `test_guide_has_no_cooldown` |
| `tests/test_guide.py` | 신규 | 등록+핑제거 · ephemeral 임베드 응답 · 드리프트 가드 3종 |

**검증 결과**: `pytest -q` → **519 passed** (516 기존 + 신규 3) · `ruff check . && ruff format --check .` → clean. 4단계 verify 전부 통과.

**계획 대비 차이**:
- 본문 문구 2차 수정(위 §본문 명세에 반영): `메이플메이트`→`메이트`, `길드원`→`디스코드 채널 유저`(`메이플스토리` 유지).
- 그 외(2파일 신규 + core.py 핑제거·배선, 6그룹·드리프트 가드·쿨다운 없음·ephemeral)는 계획과 동일.

**남은 일**: PR #24 리뷰·머지 → 배포 → 디스코드에서 `/가이드` 라이브 응답 확인(글로벌 슬래시 동기화 반영 최대 1시간).
