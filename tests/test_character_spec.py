...
def test_hexa_core_levels_grouped_by_type():
    ...
    hexa = {
        "character_hexa_core_equipment": [
            {"hexa_core_type": "스킬 코어", "hexa_core_level": 1, "hexa_core_name": "크로노"},
            {"hexa_core_type": "스킬 코어", "hexa_core_level": 4, "hexa_core_name": "바이템"},
...
def test_hexa_stat_triples_numbers_only():
    hexa_stat = {
        "character_hexa_stat_core": [
            {"main_stat_name": "크리티컬 데미지", "main_stat_level": 4, "sub_stat_level_1": 10, "sub_stat_level_2": 6}
        ],
...
class _FakeStatClient:
    ...
    async def character_stat(self, ocid, date=None):
        ...
        if date in self._not_ready:
            raise NexonAPIError("OPENAPI00009", "wait", error_class=ErrorClass.DATA_NOT_READY)
...
async def test_fetch_combat_power_on_parses_and_caches():
    ...
    cache: dict = {}
    assert await fetch_combat_power_on(client, "oc1", "2026-06-01", cache=cache) == 180000000
...
async def test_fetch_weekly_max_power_picks_max_over_window():
    ...
    got = await fetch_weekly_max_power(client, "oc1", date(2026, 6, 8), latest=145000000)
...
async def test_fetch_weekly_max_power_falls_back_to_latest():
    ...
    got = await fetch_weekly_max_power(client, "oc1", date(2026, 6, 8), latest=185000000)
...
async def test_fetch_weekly_max_power_none_when_no_data_and_no_latest():
    ...
    assert await fetch_weekly_max_power(client, "oc1", date(2026, 6, 8), latest=None) is None
